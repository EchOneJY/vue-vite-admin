...
const transform: AxiosTransform = {
  ...
  transformRequestData: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    if (code === ResultEnum.TIMEOUT) {
      ...
      const timeoutMsg = 'Login timed out, please log in again!';
      createErrorModal({
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    try {
      if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
        ...
        createMessage.error('The interface request timed out, please refresh the page and try again!');
      }
      if (err?.includes('Network Error')) {
        createErrorModal({
          title: 'network anomaly',
          // title: t('sys.api.networkException'),
          content: 'Please check if your network connection is normal! The network is abnormal',
